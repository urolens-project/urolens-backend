...
| 4 | AI integration / inference service | Rebuild: new service class modeled on `ai_integration.py`'s shape (proper class, not inline router logic), rewired onto Supabase Storage (per "Global architecture decisions"), with `src/urolens/api/image.py`'s router-inline production logic ported in (dash→underscore particle-class normalization, retry-without-`updated_at`, smart-diagnosis-at-upload precompute) | `ai_integration_service.py` outright; the inline logic currently in `src/urolens/api/image.py` (extracted into the new service, not left in the router — see scope rule above) | None of the three existing implementations survives unmodified — see full reasoning below. **Correction from the image/AI domain merge**: this row's filenames were swapped relative to their actual content. Direct inspection at merge time found `ai_integration.py` was the one with the stale schema (`ImageStatus.UPLOADED`/`ResultStatus.PENDING_REVIEW`, neither a real enum member) *and* the broken `from ..core.config import settings` import, while `ai_integration_service.py` was the schema-correct one (matches current `Image`/`AnalysisResult` models) with a working import (`from ..core.config import AI_MODEL_VERSION, S3_BUCKET`, both real flat constants). The merge kept and rewrote `ai_integration_service.py` and deleted `ai_integration.py` — the outcome this row intended (schema-correct file survives, stale one is deleted), just the opposite filename mapping from what's written above. Left uncorrected above rather than rewritten, so the audit trail shows what was believed at the time versus what was found. |
...
| 6 | Confirm / override results | `src/urolens/api/results.py` (currently commented out in `main.py`) + `ManualOverrideService`'s re-derivation logic | `app/api/results.py` (confirm/override routes only — see #7 for the rest of that file) + `app/services/result_service.py` | Confirmed: `app/services/result_service.py:136,171` takes `original_ai_value` as a caller-supplied argument and stores it as-is. `ManualOverrideService._extract_original_value` re-derives it from stored `ai_findings`, ignoring the caller's value. `src/urolens/api/results.py` also currently imports the non-canonical RBAC/UserRole pair — swap during this merge. **Two corrections from the row-6 merge**: (1) `app/services/result_service.py` could not be deleted "outright" as this row said — it also contains `get_smart_diagnosis`, used by `app/api/results.py`'s `GET /{result_id}/smart-diagnosis` route, which belongs to neither row 6 nor row 7 and stays untouched; only `confirm_result`/`override_parameter` were removed from that file. (2) `src/urolens/api/results.py` was not just missing a RBAC swap — it already contained full row-7-shaped routes (`/pending`, `/{id}/full`, `/annotate`, `/approve`, `/return`, `/escalate`, all bridged to the old, unported `app.services.result_review_service`) plus two generic result-viewing routes (`GET /{id}`, `GET /{id}/smart-diagnosis`, inline Supabase calls). Mounting the file as-is alongside the still-live `app/api/results.py` (row 7, deliberately untouched) would have silently shadowed one router's routes with the other's at nearly every path, in registration order, with no error — both share the `/api/v1/results` prefix. Those routes were removed from `src/urolens/api/results.py` before mounting (recoverable from git history for whoever does row 7); only confirm/override remain in that file. See `CHANGELOG.md` for the full route-by-route collision list. |
| 7 | **Supervisor review/approval workflow — not in the original brief's table** | Needs to be *ported*, not picked between — no SQLAlchemy equivalent exists | `app/services/result_review_service.py` (636 lines: `get_pending`, `get_approved_today`, `get_escalated`, `save_annotation`, `approve_result`, `return_result`, `escalate_result`) is live behind `app/api/results.py`, pure Supabase REST, and has **no counterpart anywhere under `src/urolens/services/`.** This is a Tier-1 workflow (approve/return/escalate is the standards skill's own named example of the confirm→override→approve→release chain) that has to be rewritten against SQLAlchemy as part of the results merge (Step 5), not deleted or kept-as-is. Size this as its own line item in that step's estimate — it's the largest single piece of work in the results domain, larger than #6. **Correction from the row-7 port (step 5b)**: this method list is incomplete — the live file also has `get_supervisor_stats` and `get_full_result` (backing `GET /supervisor/stats` and `GET /{result_id}`), 9 methods total, not 7. Both were ported along with the listed 7. Also: all four tables this service touches (`result_reviews`, `result_approvals`, `result_returns`, `escalations`) turned out to already have full Alembic history (migration `0019`) despite having no SQLAlchemy models — not the same schema-drift pattern as migrations `0032`/`0033`, so models were added with no new migration needed. Two real, narrower schema-drift findings did surface and were **not** fixed with a migration, per this consolidation's standing rule: `analysis_results.confirmation_notes` and `result_reviews.spatial_annotations`, neither with any Alembic history anywhere. See `CHANGELOG.md` for the full handling of both. |
